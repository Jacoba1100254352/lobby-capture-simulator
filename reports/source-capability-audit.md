...
| sam-contract-awards-action-history | sam-contract-awards (missing) | 0 | thin | implemented-not-active | Not active in the committed snapshot. Enable SAM_CONTRACT_AWARDS_SOURCE_NATIVE=1 with SAM_API_KEY; use SAM_CONTRACT_AWARDS_EXTRACT_MODE=1 for asynchronous extracts or department-code/PIID-subtier filters plus SAM_CONTRACT_AWARDS_OFFSET_STARTS for synchronous non-adjacent page-index slices. | Procurement modification capture and calibrated policy-simulation claims | Archive a representative SAM/FPDS action-history pull or configured export; compare modification incidence against the bounded USAspending action panel. |
| usaspending-stratified-action-panel | usaspending-procurement-actions (ok) | 2399 | usable | active-usable | normalized USAspending procurement action rows written | Bounded procurement concentration and modification diagnostics | Broaden beyond the selected 12-agency quarterly stress panel before treating modification incidence as calibration-grade. |
