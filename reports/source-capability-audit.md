...
| sam-contract-awards-action-history | sam-contract-awards (missing) | 0 | warning | implemented-not-active | Not active in the committed snapshot. Enable SAM_CONTRACT_AWARDS_SOURCE_NATIVE=1 with SAM_API_KEY; use SAM_CONTRACT_AWARDS_EXTRACT_MODE=1 for asynchronous extracts or department-code/PIID-subtier filters plus SAM_CONTRACT_AWARDS_OFFSET_STARTS for synchronous non-adjacent page-index slices. | Procurement modification capture and calibrated policy-simulation claims | Archive a representative SAM/FPDS action-history pull or configured export; compare modification incidence against the bounded USAspending action panel. |
