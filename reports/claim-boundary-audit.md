...
| OIRA EO 12866 meeting logs | Public meeting disclosure, requestor/client visibility, and access concentration | direct public meeting disclosure | thin | thin | May support source-aware plausibility checks only; magnitude claims must be phrased as proxy-backed or thin. | Do not present as article-level calibration or validation of hidden-channel magnitude. | Broaden Reginfo.gov EO 12866 meeting rows, fetch detail pages for requestor/client fields, and link requestors or clients to LDA, docket, FEC, intermediary, and procurement identifiers before estimating substitution elasticity. |
...
- Thin, warning, fixture-only, or missing panels requiring explicit coverage limits: `1`
- `OIRA EO 12866 meeting logs` (thin): May support source-aware plausibility checks only; magnitude claims must be phrased as proxy-backed or thin.
