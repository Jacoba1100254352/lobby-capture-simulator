...
- Next action: Run make sam-contract-awards-preflight immediately before the live snapshot; if the endpoint returns nextAccessTime, wait for that UTC timestamp.

## Refresh Modes

Run `make sam-contract-awards-preflight` immediately before either SAM.gov mode. The preflight makes a one-row redacted Contract Awards request and writes ignored operational reports under `reports/sam-contract-awards-preflight.*`.
