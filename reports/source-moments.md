...
| snapshot | fec | `darkMoneyRows` | 330.0000 | observed_proxy | DARK_MONEY rows from FEC or explicit dark-money/opaque-capacity bridge panels |
...
| snapshot | fec | `darkMoneyDirectRoutingSourceShare` | 0.0779 | observed | non-proxy DARK_MONEY share of normalized campaign-finance plus bridge amount |
...
| snapshot | fec | `darkMoneySourceShare` | 0.1005 | observed_proxy | DARK_MONEY or opaque-capacity bridge share of normalized campaign-finance plus bridge amount |
| snapshot | fec | `superPacSourceShare` | 0.0070 | observed_proxy | SUPER_PAC share of normalized campaign-finance plus bridge amount |
| snapshot | fec | `opaqueElectoralSourceShare` | 0.1075 | observed_proxy | DARK_MONEY plus SUPER_PAC share of normalized campaign-finance plus bridge amount |
...
| snapshot | fec | `outsideSpendingRows` | 998.0000 | observed | normalized independent expenditure, super PAC, dark-money, association, electioneering, or communication-cost rows |
| snapshot | fec | `outsideSpendingSourceShare` | 0.1178 | observed_proxy | outside-spending bridge share of normalized campaign-finance plus bridge amount |
| snapshot | fec | `outsideSpendingTop3SourceShare` | 0.6638 | observed_proxy | top three outside spenders by normalized amount |
| snapshot | fec | `outsideSpendingDisclosureLagMean` | 0.4833 | observed_proxy | amount-weighted reporting lag among outside-spending rows |
| snapshot | fec | `publicFinancingRows` | 135.0000 | observed_proxy | public-match or voucher rows from FEC or explicit public-financing panel |
| snapshot | fec | `publicFinancingProgramCount` | 2.0000 | observed_proxy | distinct public-financing program sources represented |
| snapshot | fec | `publicFinancingVoucherRows` | 3.0000 | observed | democracy-voucher rows from explicit public-financing panels |
...
| snapshot | fec | `publicFinancingVoucherAmount` | 0.0968 | observed_proxy | sum of normalized democracy-voucher amount |
| snapshot | fec | `publicFinancingProgramAmount` | 87.0526 | observed_proxy | sum of public-match and voucher bridge amount |
| snapshot | fec | `publicFinancingSourceShare` | 0.0758 | observed_proxy | public-match or voucher share of normalized campaign-finance plus bridge amount |
...
| snapshot | usaspending | `procurementActionRows` | 28104.0000 | observed | primary procurement action denominator from stratified USAspending transaction/action rows |
| snapshot | usaspending | `procurementUsaspendingActionRows` | 28104.0000 | observed | normalized USAspending transaction/action rows, if available |
...
| snapshot | usaspending | `procurementModificationPanelRows` | 28104.0000 | diagnostic | rows used for modification moments from stratified USAspending transaction/action rows |
...
| snapshot | usaspending | `procurementTotalAwards` | 194287.3483 | observed | sum of national-volume no-agency-filtered USAspending transaction/action rows amount |
...
| snapshot | usaspending | `procurementAgencyTop1CountShare` | 0.7153 | diagnostic | largest awarding agency row-count share of national-volume no-agency-filtered USAspending transaction/action rows |
...
| snapshot | usaspending | `procurementAmountWeightedSingleBidShare` | 0.2109 | observed_proxy | award-amount share among normalized USAspending award rows with competition fields with one known offer |
| snapshot | usaspending | `procurementInitialAwardShare` | 0.5777 | observed_proxy | share of stratified USAspending transaction/action rows that appear to be initial awards |
| snapshot | usaspending | `procurementExPostModificationShare` | 0.4223 | observed_proxy | share of stratified USAspending transaction/action rows marked as ex-post modifications or nonzero modification sequence |
| snapshot | usaspending | `procurementActionDistinctAwards` | 23809.0000 | diagnostic | distinct PIID/award identifiers in stratified USAspending transaction/action rows used for award-level modification diagnostics |
| snapshot | usaspending | `procurementModifiedAwardCount` | 8203.0000 | diagnostic | distinct PIID/award identifiers in stratified USAspending transaction/action rows with at least one ex-post modification row |
| snapshot | usaspending | `procurementModifiedAwardShare` | 0.3445 | observed_proxy | share of distinct PIID/award identifiers in stratified USAspending transaction/action rows with at least one ex-post modification row |
| snapshot | usaspending | `procurementModificationActionsPerModifiedAward` | 1.4467 | diagnostic | mean modified action rows per modified PIID/award identifier in stratified USAspending transaction/action rows |
...
| snapshot | usaspending | `procurementActionModificationRows` | 11867.0000 | diagnostic | primary transaction/action rows from stratified USAspending transaction/action rows marked as ex-post modifications |
...
| snapshot | intermediary | `intermediaryTotalRevenue` | 6095.4205 | observed_proxy | sum of normalized intermediary revenue |
| snapshot | intermediary | `intermediaryPoliticalSpendShare` | 0.1482 | observed_proxy | political spend share of normalized intermediary revenue |
| snapshot | intermediary | `intermediaryTop3RevenueShare` | 0.1668 | observed_proxy | top three intermediary organizations by revenue |
| snapshot | intermediary | `intermediaryDonorDisclosureMean` | 0.6716 | observed_proxy | mean donor/source disclosure score |
| snapshot | intermediary | `intermediaryAssociationShare` | 0.0946 | observed_proxy | share of rows marked 501(c)(6) association |
| snapshot | intermediary | `intermediaryC4Share` | 0.0251 | observed_proxy | share of rows marked 501(c)(4) social welfare |
...
| snapshot | intermediary | `intermediary527PoliticalSpendShare` | 0.9488 | observed_proxy | 527/IRS 8872 share of intermediary political spending |
...
| snapshot | intermediary | `intermediaryGrantmakingShare` | 0.0087 | observed_proxy | grantmaking share of normalized intermediary revenue |
