...
| doi-deposit-package | ready | package=present; manifest=present; manifest members=110; zip members=112; primary assets=5/5; zip integrity=ok; package checksum=ok | Upload or retain dist/lobby-capture-doi-deposit-package.zip as the single archive handoff package when the repository-to-archive integration does not preserve release assets directly. |
