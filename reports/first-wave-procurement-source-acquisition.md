...
| gao-protest-overlay | candidate_unreviewed | GAO Recent Bid Protest Decisions, GAO Search Decisions & Docket, and GAO Legal Products XML feed (https://www.gao.gov/legal/bid-protests/recent; https://www.gao.gov/rss/reportslegal.xml) | Protest ID or B-number, agency, filed date where available, decision date, outcome, issue codes, source URL, and manually reviewed PIID/UEI/vendor linkage when available | GAO decision surfaces do not reliably expose PIID/UEI fields in a machine-readable way, so award/vendor linkage is a manual review task. |
...
- Fallback: Run make gao-protest-feed-preflight against the GAO Legal Products XML feed for discovery, then review decision pages or PDFs for extracted fields
...
- Acquisition step: Run make gao-protest-feed-preflight to build an ignored GAO protest discovery worklist, extract decision metadata from the official decision pages, then manually link decisions to PIID, UEI, vendor, agency, and issue fields before promotion.
