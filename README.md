...
- Reginfo.gov EO 12866 meeting logs (no key): <https://www.reginfo.gov/public/do/eom12866Search>
...
make procurement-causal-upgrade-packet
make substitution-estimation-diagnostics
make substitution-causal-upgrade-packet
make comment-causal-upgrade-packet
make venue-causal-upgrade-packet
```

The acquisition report is an acquisition plan, not evidence. It maps the first-wave procurement source products to official public surfaces: SAM.gov Contract Awards for action history and offer-count/competition enrichment, GAO bid protest decisions and Legal Products feed for protest overlays, SAM.gov Exclusions API or public exclusion extracts for exclusion overlays, and agency procurement-integrity documents for broader firewall coverage. The procurement causal-upgrade packet consolidates those products with the causal target, denominator separation, manual promotion blockers, and claim boundary. The committed EPAAR Part 1503 row is a reviewed but bounded procurement-firewall control-design product; it does not clear SAM/FPDS action history, protest, exclusion, offer-count, broader firewall, or causal procurement-modification gates. Populate the named files under `data/calibration/first-wave/`, then rerun `make first-wave-source-products`, `make first-wave-source-readiness`, `make procurement-causal-upgrade-packet`, and `make paper-artifacts-check` before treating any stronger procurement claim as source-supported.

The substitution causal-upgrade packet is the matching control artifact for the central P1 substitution-elasticity target. The reviewed HLOGA source products now supply treated federal LDA rows and Colorado unaffected-jurisdiction controls, and `make substitution-estimation-diagnostics` builds the first actor-quarter descriptive estimator, pre-trend/placebo/window/outlier checks, leave-one-actor sensitivity, and an explicit no-claim report. That diagnostic reports `effect_model_and_falsification_gates_not_cleared`: treatment is confounded with source and jurisdiction, outcome and issue definitions are not comparable, only two clean pre quarters exist, the sole clean placebo is not independent, the actor-bootstrap interval crosses zero, and one actor omission reverses the descriptive sign. It therefore does not clear a causal HLOGA effect, cross-channel substitution elasticity, or calibrated policy-simulation claim. After editing substitution source products, rerun `make first-wave-source-products`, `make first-wave-source-readiness`, `make first-wave-manual-adjudication-plan`, `make substitution-estimation-diagnostics`, `make substitution-causal-upgrade-packet`, and `make paper-artifacts-check`.

The comment causal-upgrade packet is the matching control artifact for the P2 comment-authenticity and uptake target. It consolidates the comment-body corpus, duplicate/template cluster scaffold, candidate response/final-rule linkage blocker, and no-causal-clearance boundary. It is not source evidence and does not clear causal comment-uptake or calibrated policy-simulation claims; after editing comment source products, rerun `make first-wave-source-products`, `make first-wave-source-readiness`, `make first-wave-manual-adjudication-plan`, `make comment-causal-upgrade-packet`, and `make paper-artifacts-check`.

The September 2026 empirical expansion is documented in [the substitution redesign](docs/substitution-study-redesign.md), [comment-uptake pilot](docs/comment-uptake-pilot.md), and [procurement reconciliation](docs/procurement-source-reconciliation.md). It adds 2003-2008 LDA history, a bounded PAC report/half-year outcome series, and two source-reviewed docket-issue observations. It also repairs the 500-comment text corpus and separates GAO publication dates from decision dates. Run `make empirical-expansion-audit` for the offline evidence checks. These products do not assign a valid treatment/control contrast or clear causal-calibration claims.
...
- `make mechanism-review-circulation-readiness` writes `reports/mechanism-review-circulation-readiness.{csv,md}` after the DOI readiness audit. It is the compact deterministic gate for the current objective: mechanism-review circulation is ready only when automated artifact, build, packaging, reviewer-risk, candidate-source, and claim-boundary checks pass while calibrated policy-effect and final journal-submission claims remain separated unless their prerequisites clear. This verifier is intentionally kept outside the Wiley ZIP because it inspects post-package artifacts.
