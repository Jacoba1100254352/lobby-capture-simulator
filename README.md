...
For an emailed SAM.gov async-export link, paste the full URL into `.env` as `SAM_CONTRACT_AWARDS_LIVE_URL` and leave `api_key=REPLACE_WITH_API_KEY` in that URL; keep the private key only in `SAM_API_KEY`. `make sam-contract-awards-export-audit` substitutes the key at runtime and redacts both key and token from diagnostics. `make external-finalization-checklist` summarizes whether a SAM export URL/CSV, GitHub release asset audit, Zenodo token/draft/upload state, DOI record, human read-through, and live journal-guideline refresh are ready, manual, or blocked. Its reports are ignored because they can reflect private `.env` and live release state.
...
- `make external-finalization-checklist` writes ignored `reports/external-finalization-checklist.{csv,md}` after refreshing the DOI deposit readiness audit. It sources `.env`, folds in the post-release GitHub asset audit if present, records Zenodo draft/upload state if present, checks whether a DOI has been recorded in repository metadata, and reports SAM.gov export-input readiness without leaking keys or emailed export tokens.
