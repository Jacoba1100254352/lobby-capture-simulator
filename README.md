...
It preserves raw public API payloads under ignored `data/raw/source-payloads/2024-env/`, writes normalized rows into `data/raw/`, runs the snapshot freezer, and emits source moments. If no personal API keys are configured it uses official public/demo access where possible and records rate-limit gaps in `data/snapshots/2024-env/live-run-status.csv`. Set `SAM_CONTRACT_AWARDS_LIVE_CSV` or `SAM_CONTRACT_AWARDS_LIVE_URL` to normalize a downloaded SAM.gov Contract Awards CSV/JSON/ZIP export as the primary procurement action panel. SAM.gov emailed async-extract links can be pasted into `SAM_CONTRACT_AWARDS_LIVE_URL` with `api_key=REPLACE_WITH_API_KEY`; the local `SAM_API_KEY` is substituted at runtime and redacted in diagnostics. Because emailed SAM.gov download links are short-lived, also set `SAM_CONTRACT_AWARDS_LIVE_URL_GENERATED_AT` or `SAM_CONTRACT_AWARDS_LIVE_URL_EXPIRES_AT` so the audit can fail closed before trying an expired token. If no export is configured, set `SAM_CONTRACT_AWARDS_SOURCE_NATIVE=1` with `SAM_API_KEY` to try the SAM.gov Contract Awards API; the runner records SAM availability and falls back to USAspending action rows if that opt-in request fails.
...
For an emailed SAM.gov async-export link, paste the full URL into `.env` as `SAM_CONTRACT_AWARDS_LIVE_URL` and leave `api_key=REPLACE_WITH_API_KEY` in that URL; keep the private key only in `SAM_API_KEY`. Add either `SAM_CONTRACT_AWARDS_LIVE_URL_GENERATED_AT=YYYY-MM-DDTHH:MM:SSZ` plus `SAM_CONTRACT_AWARDS_LIVE_URL_VALID_MINUTES=60`, or set `SAM_CONTRACT_AWARDS_LIVE_URL_EXPIRES_AT` directly. `make sam-contract-awards-export-audit` substitutes the key at runtime, redacts both key and token from diagnostics, and treats an explicitly expired emailed link as a manual-refresh condition rather than a promotable source. `make external-finalization-checklist` summarizes whether a SAM export URL/CSV, GitHub release asset audit, Zenodo token/draft/upload state, DOI record, human read-through, and live journal-guideline refresh are ready, manual, or blocked. Its reports are ignored because they can reflect private `.env` and live release state.
