...
- `make external-finalization-checklist` writes ignored `reports/external-finalization-checklist.{csv,md}` from the latest local readiness reports and live/private environment state. It sources `.env`, folds in the post-release GitHub asset audit if present, records Zenodo draft/upload state if present, checks whether a DOI has been recorded in repository metadata, and reports SAM.gov export-input readiness without leaking keys or emailed export tokens. Run `make paper-artifacts-check` separately before creating or replacing release assets.
