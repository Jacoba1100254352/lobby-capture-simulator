...
`make first-wave-source-products` writes `reports/first-wave-source-products.csv` plus `reports/first-wave-source-products.md`. It converts the first-wave causal protocols into executable source-product schemas under `data/calibration/first-wave/`, including expected paths, required columns or text terms, acceptable source families, validation rules, and claim boundaries. `make first-wave-source-readiness` consumes that schema gate before any first-wave protocol can move from design-ready to estimation-ready.
