...
- Confirm the Wiley submission archive includes `supporting-information/final-human-readthrough.md` and `supporting-information/final-human-readthrough-audit.md`; these files record final human editorial signoff and the structured audit that prevents a stale or incomplete signoff from clearing.
...
- After uploading the GitHub release assets, run `make github-release-asset-audit`. This networked post-release check writes ignored `reports/github-release-asset-audit.{csv,md}` and compares GitHub's uploaded asset sizes and SHA-256 digests against `dist/release-asset-checksums.*`, `dist/doi-deposit-package-checksum.*`, and the generated release-aid reports.
- After pushing main and the release tag, run `make github-ci-status-audit`. This networked post-release check writes ignored `reports/github-ci-status-audit.{csv,md}` and verifies that GitHub Actions completed successfully for both refs.
