...
| USAspending | Direct observed procurement | prime awards, recipient/agency concentration, award types, PIID fallback, single-bid, competition, offer-count, initial-award status, multi-agency top-award bridge diagnostics, and transaction/action rows where available | no key currently required; `USASPENDING_API_BASE` | Implemented source-native parser/fetcher for awards, top-award bridge, and a separate stratified procurement action panel. Source moments prefer the action panel for concentration when it has sufficient multi-agency coverage, retain the top-award bridge as a separate high-value diagnostic, and prefer action rows for modification incidence when present. The denominator audit reports row-count balance against amount concentration so the stratified action sample cannot be mistaken for a volume-representative SAM/FPDS denominator. Latest-transaction enrichment remains directional context when the action panel is absent |
