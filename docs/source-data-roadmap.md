...
| SAM.gov/FPDS | Direct observed procurement | UEI/PIID matching, contract awards, exclusions, action-level modifications | `SAM_API_KEY`; <https://open.gsa.gov/api/contract-awards/> | Optional SAM.gov Contract Awards fetcher implemented for the procurement action schema with PIID, UEI, modification, competition, offer-count, department, awardee, date, and obligation fields where returned. It is opt-in and fallback-protected; representative SAM/FPDS action-history expansion is still needed for calibration-grade transaction denominators |
...
5. Broaden the procurement bridge from the EPA/USAspending slice, implemented multi-agency top-award bridge, stratified USAspending procurement action panel, and optional SAM.gov Contract Awards importer into representative SAM/FPDS action-history coverage with exclusions, action-level modifications, award-action dates, protest links, and firewall coverage for procurement firewalls, ex-post modifications, concentration, and single-bid or price-only risks.
