...
5. Broaden the procurement bridge from the EPA/USAspending slice, implemented multi-agency top-award bridge, and stratified USAspending procurement action panel into representative SAM/FPDS action-history coverage with exclusions, action-level modifications, award-action dates, protest links, and firewall coverage for procurement firewalls, ex-post modifications, concentration, and single-bid or price-only risks.
