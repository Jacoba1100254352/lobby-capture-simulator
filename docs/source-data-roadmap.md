...
5. Broaden the procurement bridge from the EPA/USAspending slice, implemented multi-agency top-award bridge, stratified USAspending procurement action panel, and optional separate SAM.gov Contract Awards panel into representative SAM/FPDS action-history coverage with exclusions, action-level modifications, award-action dates, protest links, and firewall coverage for procurement firewalls, ex-post modifications, concentration, and single-bid or price-only risks. The immediate keyed-refresh path should use multiple agency or department filters plus non-adjacent `SAM_CONTRACT_AWARDS_OFFSET_STARTS` so the archived action-history pull is not just a leading-page sample.
