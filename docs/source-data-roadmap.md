...
| SAM.gov/FPDS | Direct observed procurement | UEI/PIID matching, contract awards, exclusions, action-level modifications | `SAM_API_KEY`; <https://open.gsa.gov/api/contract-awards/> | Optional SAM.gov Contract Awards fetcher implemented as a separate procurement action panel with PIID, UEI, modification, competition, offer-count, department, awardee, date, and obligation fields where returned. It uses documented department-code filters, optional PIID-subtier filters, non-overlapping offset page indexes, optional non-adjacent page-index strata through `SAM_CONTRACT_AWARDS_OFFSET_STARTS`, and opt-in timeout/retry controls, then falls back to USAspending action rows if the endpoint is unavailable. Source moments keep SAM and USAspending row counts separate while selecting one primary action denominator. Representative SAM/FPDS action-history expansion is still needed for calibration-grade transaction denominators |
...
5. Broaden the procurement bridge from the EPA/USAspending slice, implemented multi-agency top-award bridge, stratified USAspending procurement action panel, and optional separate SAM.gov Contract Awards panel into representative SAM/FPDS action-history coverage with exclusions, action-level modifications, award-action dates, protest links, and firewall coverage for procurement firewalls, ex-post modifications, concentration, and single-bid or price-only risks. The immediate keyed-refresh path should use multiple agency, department-code, or PIID-subtier filters plus non-adjacent `SAM_CONTRACT_AWARDS_OFFSET_STARTS` page indexes so the archived action-history pull is not just a leading-page sample.
