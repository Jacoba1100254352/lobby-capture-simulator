...
8. Run `make procurement-refresh-readiness` before any keyed SAM.gov refresh to confirm the configured acquisition path, quota/reset status, fallback policy, and claim boundary without spending API quota.
9. Rerun `make snapshot-2024-env source-moments validate calibration-queue procurement-refresh-readiness paper-artifacts-check` and commit regenerated artifacts.
