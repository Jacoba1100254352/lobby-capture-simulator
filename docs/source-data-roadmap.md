...
| USAspending | Direct observed procurement | prime awards, recipient/agency concentration, award types, PIID fallback, single-bid, competition, offer-count, initial-award status, multi-agency top-award bridge diagnostics, and transaction/action rows where available | no key currently required; `USASPENDING_API_BASE` | Implemented source-native parser/fetcher for awards, top-award bridge, a separate expanded stratified procurement action panel, and a separate no-agency-filtered national-volume action panel. Source moments prefer SAM.gov action rows when archived, otherwise the national-volume action panel for concentration if present, while retaining the balanced action panel for modification incidence and the top-award bridge as a high-value diagnostic. The denominator audit reports row-count balance against amount concentration so neither action sample can be mistaken for a representative SAM/FPDS modification denominator. Latest-transaction enrichment remains directional context when the action panel is absent |
| SAM.gov/FPDS | Direct observed procurement | UEI/PIID matching, contract awards, exclusions, action-level modifications | `SAM_API_KEY`; <https://open.gsa.gov/api/contract-awards/> | Optional SAM.gov Contract Awards fetcher implemented as a separate procurement action panel with PIID, UEI, modification, competition, offer-count, department, awardee, date, and obligation fields where returned. It uses documented department-code filters, optional PIID-subtier filters, synchronous non-overlapping offset page indexes, optional non-adjacent page-index strata through `SAM_CONTRACT_AWARDS_OFFSET_STARTS`, and asynchronous JSON/CSV extract downloads through `SAM_CONTRACT_AWARDS_EXTRACT_MODE`, then falls back to USAspending action rows if the endpoint is unavailable. Source moments keep SAM and USAspending row counts separate while selecting one primary action denominator. Representative SAM/FPDS action-history expansion is still needed for calibration-grade transaction denominators |
...
5. Broaden the procurement bridge from the EPA/USAspending slice, implemented multi-agency top-award bridge, expanded stratified USAspending procurement action panel, and optional separate SAM.gov Contract Awards panel into representative SAM/FPDS action-history coverage with exclusions, action-level modifications, award-action dates, protest links, and firewall coverage for procurement firewalls, ex-post modifications, concentration, and single-bid or price-only risks. The immediate keyed-refresh path should use `SAM_CONTRACT_AWARDS_EXTRACT_MODE=1` for larger asynchronous JSON/CSV extracts, or multiple agency, department-code, or PIID-subtier filters plus non-adjacent `SAM_CONTRACT_AWARDS_OFFSET_STARTS` page indexes for bounded synchronous samples. If the initial extract-token request times out before a `presignedUrl` is returned, verify that the SAM.gov key/account has Contract Awards API access, required role permissions, allowed network origin, and any 429 `nextAccessTime` before treating the failure as a model/data result. Keep `SOURCE_FETCH_CURL_FALLBACK=1` enabled when SAM responds to curl but hangs under the Python request path.
...
8. Run `make procurement-refresh-readiness` before any keyed SAM.gov refresh to confirm the configured acquisition path, fallback policy, and claim boundary without spending API quota.
9. Run `make sam-contract-awards-preflight` to make a one-row redacted SAM.gov Contract Awards access/quota check. If it reports `quota_blocked`, wait for the reported UTC reset before running the live snapshot; if it reports `ok`, run the representative refresh settings immediately.
10. Rerun `make snapshot-2024-env source-moments validate calibration-queue procurement-refresh-readiness paper-artifacts-check` and commit regenerated artifacts.
