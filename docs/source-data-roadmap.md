...
5. Broaden the procurement bridge from the EPA/USAspending slice, implemented multi-agency top-award bridge, and procurement action-panel parser into representative SAM/FPDS action-history coverage with exclusions, action-level modifications, award-action dates, protest links, and firewall coverage for procurement firewalls, ex-post modifications, and single-bid or price-only risks.
