...
- USAspending modification handling: `USASPENDING_TREAT_LATEST_TRANSACTION_AS_MODIFICATION=0` keeps award-level rows from being marked as ex-post modifications just because the latest enriched transaction is a modification. Prefer the separate `usaspending-actions` fetcher or a normalized `usaspending-actions` CSV when estimating action-level modification incidence.
