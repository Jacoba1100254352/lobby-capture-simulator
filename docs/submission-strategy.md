...
- `make blinded-review-package` creates `dist/lobby-capture-wiley-blinded-review.zip` with an anonymous main manuscript, anonymous supplement, separate title page, selected supporting information, generated tables/figures, bibliography, Wiley support files, and a package-member checksum manifest. The checker compiles the extracted package and scans review-facing source/PDF text for identifying author, email, repository, or release-tag tokens.
...
- `make reggov-guidelines-readiness-audit` checks the locally verifiable Regulation & Governance/Wiley guideline surface after the Wiley upload-form and blind-review-package audits: journal target metadata, original-article wrapper, preferred and normal word limits, the 150-word abstract cap, extractable abstract and keywords, separate title-page metadata, double-anonymized package readiness, data/code availability, AI/funding/conflict statements, figures, tables, supporting information, and LaTeX package contents. It remains separate from the ZIPs and records whether the live journal author-page refresh has been completed for the current release.
- `make doi-deposit-package` builds a single ignored DOI handoff ZIP under `dist/`, a member-level manifest, and a package-level checksum record containing the primary release assets, release-machine checksums, citation metadata, journal-readiness reports, and final human-read-through checklist. `make doi-deposit-readiness-audit` remains outside that package so it can verify the package after construction.
- `make paper-artifacts` is the normal post-edit refresh path: it reruns report sweeps, source moments, validation, calibration queue, generated tables, generated figures, the local PDF, the Wiley PDF, word count, the full Wiley source package, and the blinded review package.
- `make paper-artifacts-check` is the PDF/submission guard: it runs the full refresh, verifies the local and Wiley PDFs exist and are newer than their paper inputs, checks both submission ZIPs, compiles the extracted source packages from their roots, and screens the Wiley PDF for generic template placeholder text.
...
- author, affiliation, country, and corresponding email are present in the local article, Wiley wrapper, and separate title page;
- the double-anonymized review package uses anonymous main-manuscript and supplement wrappers with a redacted data/code statement, while the title page remains separate for editorial-office use;
...
- CI runs Java 21 tests and `make paper-artifacts-check`, which rebuilds report snapshots, validation outputs, generated tables, generated figures, the local PDF, the Wiley PDF, word count, the full Wiley submission package, and the blinded review package before checking PDF freshness, package contents, redaction, and standalone package compilation. CI then runs `git diff --exit-code` so regenerated tracked artifacts cannot drift silently.

Generic Wiley submission guidance was rechecked on 2026-06-14. It says LaTeX submissions can use a ZIP archive containing the root `.tex`, compiled PDF, bibliography, class/package files, figures, tables, and other manuscript materials; Wiley also lists 256 characters as the file-name limit, 500 MB as the combined-file upload limit, requires data availability statements for research- and synthesis-based articles, and notes that executable/script upload formats are not supported. Wiley's LaTeX template page also says journal-level instructions should be checked on Wiley Online Library, and Wiley's AI guidance requires disclosure of AI-technology use. The live Regulation & Governance author page records double-anonymized review and separate title-page/main-text expectations. `reports/reggov-guidelines-readiness.md` records the local checks against those requirements and the live Regulation & Governance author-page browser check. Before live submission, re-check the Regulation & Governance author page in Wiley Online Library under Contribute because journal-specific instructions can change.

On this machine, the missing Wiley-template packages have been installed into the user TeX tree with `make wiley-tex-deps`; `make paper-artifacts-check` should be treated as the reproducibility check for the local PDF, Wiley wrapper, generated evidence, full source package, and blinded review package.
...
- Preserve the separate blinded review package and title page for double-anonymized review.
