...
- `make paper-artifacts-check` is the pre-commit and CI guard: it runs the full refresh, verifies the local and Wiley PDFs exist and are newer than their paper inputs, checks the submission zip contents, screens the Wiley PDF for generic template placeholder text, and fails if tracked generated reports, tables, or figures changed.
...
- CI runs Java 21 tests and `make paper-artifacts-check`, which rebuilds report snapshots, validation outputs, generated tables, generated figures, the local PDF, the Wiley PDF, word count, and the Wiley submission package before checking that tracked generated artifacts are clean.
