...
- `make first-wave-entity-resolution-seeds` turns the linkage-candidate report into candidate-only source-product seed files under `data/calibration/first-wave/` without marking them estimation-ready. It now also writes candidate-only `actor-issue-time-spine.csv` and `substitution-comparison-groups.csv` so substitution-elasticity review can start from concrete files while remaining blocked until manual adjudication.
...
`docs/source-product-templates/first-wave/` contains acquisition templates for those protocols. The templates are not evidence and intentionally do not live under `data/calibration/first-wave/`. `reports/first-wave-source-products.md` is the schema/acquisition gate for production source products: it names expected paths, template paths, required columns or text terms, acceptable source families, minimum-row thresholds, field-level quality checks, product-level semantic gates, validation rules, manual promotion checklists, and claim boundaries. `reports/first-wave-source-readiness.md` consumes that gate and now separates truly missing products from candidate-only unadjudicated products and schema/quality issue products; use both reports to keep design-ready protocols separate from estimation-ready source panels.
`reports/first-wave-linkage-candidates.md` is the candidate worklist for the cross-venue entity-resolution spine. It is generated from committed normalized snapshot rows and should be used to seed manual alias review, not as linkage evidence by itself. The report now separates P1/P2/P3 manual-review candidates and records `linkageEvidenceClass`, `reviewPriorityScore`, and `reviewRiskFlags` so shared-identifier and cross-venue candidates can be reviewed before weaker name-only rows. The deterministic files in `data/calibration/first-wave/canonical-actor-identifiers.csv`, `alias-resolution-audit-sample.csv`, `issue-code-crosswalk.csv`, `false-match-review-log.csv`, `linked-actor-issue-venue-time.csv`, `actor-issue-time-spine.csv`, and `substitution-comparison-groups.csv` are candidate-only review seeds derived from that report. The comment-authenticity protocol also has a candidate-only `agency-response-final-rule-linkage.csv` worklist derived from the committed comment corpus and duplicate/template clusters. These files are intentionally marked `candidate_unreviewed` so the source-product audit can show concrete progress while preserving the claim boundary. Use the audit's manual promotion checklist column before removing candidate markers from any source product.
