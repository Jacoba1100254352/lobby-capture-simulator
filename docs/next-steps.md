...
- archive a representative SAM/FPDS action-history pull using a downloaded Contract Awards/DataBank CSV/JSON/ZIP export through `SAM_CONTRACT_AWARDS_LIVE_CSV` or `SAM_CONTRACT_AWARDS_LIVE_URL`, run `make sam-contract-awards-export-audit` to screen row count, agency breadth, date span, PIID/UEI coverage, competition fields, and modification denominators, or after a successful SAM preflight use the optional SAM Contract Awards extract path when an extract email is configured, or a bounded offset/department-code diagnostic pull when extract mode is unavailable; compare its UEI, PIID, action-level modification, exclusion, award-action, and protest coverage against the stratified USAspending action panel;
...
- treat the remaining P1 procurement source gap as source-acquisition work, not model tuning: agency and recipient concentration now use the national-volume USAspending action panel, but ex-post modification incidence still needs representative SAM/FPDS action-history rows before calibrated policy-simulation claims are available;
