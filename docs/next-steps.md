...
- `make causal-calibration-targets` generates the causal target matrix, including allowed current claims, barred claims, and upgrade triggers that must clear before the project can use calibrated policy-simulation language.
...
- `make first-wave-source-product-templates` writes header/design-note templates under `docs/source-product-templates/first-wave/`; `make first-wave-source-products` translates those protocols into concrete source-product schemas under `data/calibration/first-wave/`.
- `make first-wave-entity-resolution-seeds` turns the linkage-candidate report into candidate-only source-product seed files under `data/calibration/first-wave/`. The reviewed-product promotion path now preserves those excluded rows while overlaying exact-ID links, treated federal LDA pre/post rows, and Colorado unaffected-jurisdiction controls into the actor-time and comparison-group products.
- `make first-wave-comment-linkage-seeds` builds candidate-only `agency-response-final-rule-linkage.csv` rows from the committed comment corpus and duplicate/template clusters. This creates a concrete response/final-rule uptake-coding worklist, but it remains blocked until response sections, final-rule text movement, and uptake codes are manually reviewed against observed agency materials.
- `make first-wave-source-readiness` maps those protocols and source-product schemas to committed source products, bounded/proxy support, missing products, blocking issues, and next source actions.
- `make substitution-estimation-diagnostics` builds a balanced actor-quarter preparation panel and runs the first HLOGA descriptive contrast, pre-trend, placebo, window, outlier, and leave-one-actor checks. Its current verdict is `effect_model_and_falsification_gates_not_cleared`, so it is failure-mode evidence rather than a causal substitution estimate.
...
- for emailed SAM.gov async-export links, store the URL in `.env` as `SAM_CONTRACT_AWARDS_LIVE_URL` with `api_key=REPLACE_WITH_API_KEY`, keep the private credential in `SAM_API_KEY`, run `make external-finalization-checklist`, then use `make sam-procurement-refresh` so the export is audited and promoted only if `reports/sam-contract-awards-export-audit.csv` reports `promotion-readiness=candidate`;
- keep `SAM_CONTRACT_AWARDS_ALLOW_DIAGNOSTIC_PROMOTION=0` for publication refreshes; diagnostic exports have coverage warnings and should not be promoted unless a diagnostic-only refresh is explicitly documented;
...
`reports/causal-calibration-targets.md` is stricter than the validation queue and now separates first-wave empirical upgrades from second-wave coverage work, with an explicit allowed-current-claim/barred-claim matrix for each open target. Treat the first-wave rows as the next publication-grade empirical agenda because they are the shortest route from public/source panels to stronger manuscript claims without changing the current mechanism-model boundary.
...
`docs/source-product-templates/first-wave/` contains acquisition templates for those protocols. The templates are not evidence and intentionally do not live under `data/calibration/first-wave/`. `reports/first-wave-source-products.md` is the schema/acquisition gate for production source products: it names expected paths, template paths, required columns or text terms, acceptable source families, minimum-row thresholds, field-level quality checks, product-level semantic gates, validation rules, manual promotion checklists, and claim boundaries. `reports/first-wave-source-readiness.md` consumes that gate. For substitution, the source-product gate is now ready for design use, but the separate estimator packet shows why source readiness does not equal effect identification.
`reports/first-wave-linkage-candidates.md` remains the candidate worklist for broader cross-venue entity resolution. The promoted actor-time and comparison-group products now combine reviewed exact-ID rows, treated LDA source rows, Colorado control rows, and explicitly excluded candidate rows. `reports/substitution-estimation-diagnostics.md` is the controlling effect-model handoff: it documents source-system confounding, incompatible outcome semantics, issue-taxonomy mismatch, inadequate pre-period and placebo depth, interval uncertainty, and leave-one-actor instability. The comment-authenticity protocol still has a candidate-only `agency-response-final-rule-linkage.csv` worklist derived from the committed comment corpus and duplicate/template clusters. Use `reports/first-wave-manual-adjudication-plan.md` for remaining product-level blockers and row-level review batches.
...
- use the P1 rows in the candidate-only entity-resolution seed files to select high-value actor names for manual `canonicalActorId`, alias-decision, false-match, issue-crosswalk, and linked actor-issue-venue-time adjudication;
- use the candidate response/final-rule linkage file to review agency response sections, final-rule text movement, and uptake codes before estimating comment-authenticity effects;
- start from the acquisition templates in `docs/source-product-templates/first-wave/`, then add populated first-wave products under `data/calibration/first-wave/` using the schemas in `reports/first-wave-source-products.md`; rerun the source-product and source-readiness gates before inspecting estimates;
- clear the first-wave source-readiness audit, then separately clear the matching estimator, falsification, sensitivity, and external-review gates before changing the manuscript's claim posture;
- replace the source-confounded HLOGA comparison with a longer within-LDA matched-control design and add observed alternate-channel outcomes before interpreting any coefficient as substitution;
