...
`docs/source-product-templates/first-wave/` contains acquisition templates for those protocols. The templates are not evidence and intentionally do not live under `data/calibration/first-wave/`. `reports/first-wave-source-products.md` is the schema/acquisition gate for production source products: it names expected paths, template paths, required columns or text terms, acceptable source families, field-level quality checks, validation rules, and claim boundaries. `reports/first-wave-source-readiness.md` consumes that gate; use both reports to keep design-ready protocols separate from estimation-ready source panels.
