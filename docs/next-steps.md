...
- `make first-wave-source-products` translates those protocols into concrete source-product schemas under `data/calibration/first-wave/`.
...
`reports/first-wave-source-products.md` is the schema/acquisition gate for those protocols: it names expected paths, required columns or text terms, acceptable source families, validation rules, and claim boundaries. `reports/first-wave-source-readiness.md` consumes that gate; use both reports to keep design-ready protocols separate from estimation-ready source panels.
...
- add first-wave products under `data/calibration/first-wave/` using the schemas in `reports/first-wave-source-products.md`, then rerun the source-product and source-readiness gates before inspecting estimates;
