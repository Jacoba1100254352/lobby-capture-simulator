...
`reports/first-wave-linkage-candidates.md` is the candidate worklist for the cross-venue entity-resolution spine. It is generated from committed normalized snapshot rows and should be used to seed manual alias review, not as linkage evidence by itself.
...
- use the linkage-candidate report to select high-value actor names for manual `canonicalActorId`, alias-decision, false-match, and linked actor-issue-venue-time source products;
