...
- run and archive a representative SAM/FPDS action-history pull using the optional SAM Contract Awards extract path, first verifying SAM.gov Contract Awards API role/access if the extract-token request times out before returning a `presignedUrl`, then compare its UEI, PIID, action-level modification, exclusion, award-action, and protest coverage against the stratified USAspending action panel;
