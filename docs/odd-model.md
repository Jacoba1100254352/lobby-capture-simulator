...
- USAspending rows constrain procurement-recipient and awarding-agency concentration. The archived USAspending bulk transaction summary supplies the preferred public transaction-history denominator for configured agencies, while the national-volume no-agency-filtered and balanced USAspending action panels remain diagnostic fallbacks. The normalized procurement-action schema also carries UEI, PIID, action date, competition type, number of offers, initial-award versus modification status, price-only, protest, exclusion, and firewall flags when USAspending transaction rows or the optional separate SAM.gov Contract Awards panel provide SAM/FPDS-style fields. The SAM path can use non-adjacent offset page-index strata for bounded synchronous refreshes or asynchronous JSON/CSV extract downloads for larger pulls; those rows are now most important for exclusions, offer counts, protest overlays, broader firewall coverage, and crosswalking USAspending modification coding.
...
Procurement contests use both simulated channel pressure and source-panel procurement risk. The bridge risk is computed from recipient concentration, single-bid or limited-competition share, post-award modification diagnostics, price-only awards, protest flags, exclusion flags, and missing or partial firewall coverage. Source moments keep award rows, top-award bridge rows, national-volume transaction/action rows, balanced transaction/action rows, archived bulk USAspending transaction summaries, optional SAM.gov Contract Awards rows, and the bounded EPAAR firewall-control row separate. The archived USAspending bulk summary supplies the preferred public concentration and modification denominator when present. Modification diagnostics are reported three ways when action rows are present: transaction-row share, distinct-award share, and amount-weighted share. SAM/FPDS rows remain useful for coding crosswalks, exclusions, offer counts, protest overlays, broader firewall coverage, and stronger domain-specific validation. Award rows remain a fallback and are reported with coverage warnings when dominated by initial awards.
...
- enforcement capacity, broad capture detection, narrow reporting-error detection, campaign sanction incidence, broad capture sanctions, and backlog;
...
The review model is strongest as a comparative mechanism prototype. The 2024 EPA/ENV source snapshot is not yet representative for hidden-donor identity evidence, all-organization nonprofit-routing networks, public-financing panels, intermediary networks, or SAM/FPDS procurement coding crosswalks. The archived USAspending bulk summary and procurement benchmark crosswalk improve the public procurement denominator, but calibrated procurement-modification capture claims still require SAM/FPDS coding reconciliation, protest/exclusion/firewall overlays, and independent causal calibration evidence. The revolving-door bridge now uses LDA-derived covered-position rows, but that is narrower than a true post-employment movement panel. The intermediary panel remains bounded by local campaign-intermediary rows, capacity proxies, IRS 527 rows, and the separate top-EIN Schedule I slice unless broader documented exports are configured. Strong-formal-control scenarios are used as comparison points, so the paper emphasizes total distortion, substitution diagnostics, network-path diagnostics, and robustness fields rather than definitive reform rankings.
