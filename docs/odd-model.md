...
Procurement contests use both simulated channel pressure and source-panel procurement risk. The bridge risk is computed from recipient concentration, single-bid or limited-competition share, post-award modification diagnostics, price-only awards, protest flags, exclusion flags, and missing firewall coverage. Source moments keep award rows, top-award bridge rows, national-volume transaction/action rows, balanced transaction/action rows, archived bulk USAspending transaction summaries, and optional SAM.gov Contract Awards rows separate. The archived USAspending bulk summary supplies the preferred public concentration and modification denominator when present. Modification diagnostics are reported three ways when action rows are present: transaction-row share, distinct-award share, and amount-weighted share. SAM/FPDS rows remain useful for coding crosswalks, exclusions, offer counts, protest/firewall overlays, and stronger domain-specific validation. Award rows remain a fallback and are reported with coverage warnings when dominated by initial awards.
