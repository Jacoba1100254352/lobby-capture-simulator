...
Use `make paper-artifacts` after manuscript, report, table, figure, or bibliography edits. It refreshes the report snapshots, validation outputs, generated tables, generated figures, local PDF, Wiley PDF, supplement PDF, word count, layout audit, and submission zip in one pass. Use `make paper-artifacts-check` before submission-facing work; it performs the same refresh and verifies the root PDFs, supplement PDF, layout audit, and submission zip. The root PDFs and submission zip are ignored build artifacts, but this check guarantees they are regenerated locally and in CI whenever their inputs change.

Paper table selection is defined in `paper/tables.yml`; generated files in `paper/tables/` include a one-line provenance comment with the source report snapshot and config path. The current table set includes mechanism comparison, selected and full campaign snapshots, apparent-failure and substitution-warning rankings, source-panel coverage, sensitivity, ablation, portfolio, and interaction snapshots, plus a static composite-weight table. `make figures` regenerates the grouped channel-mix, evasion-sensitivity, scenario-tradeoff, interaction-tradeoff, and substitution-warning-map SVG sources, Wiley-preferred PDF graphics, and LaTeX wrappers under `paper/figures/`. Inkscape must be available on `PATH` for SVG-to-PDF conversion.
