...
The primary reform criterion is total influence distortion, not observed capture alone. A reform is treated as a possible failure when observed capture falls but one or more hidden diagnostics rises:
...
This diagnostic is implemented in \texttt{scripts/validate-reports.py}. The generated outputs are \texttt{reports/substitution-audit.csv}, \texttt{reports/substitution-audit.md}, plus the substitution-failure table and map in the main manuscript.
...
\begin{itemize}
  \item campaign, sensitivity, ablation, interaction, and portfolio CSV and Markdown reports;
  ...
  \item substitution audit with possible-failure and tradeoff labels;
...
\begin{itemize}
  ...
  \item A possible reform failure requires more than channel movement. The audit flags a row when observed or visible capture falls while hidden influence, hidden capture, total distortion, or substitution risk rises.
\end{itemize}

The source-facing parameters and benchmark ranges are maintained in \texttt{data/calibration/parameter-map.csv}. Report-level bounded diagnostics remain synthetic unless the validation summary classifies the target as a source moment.

\section{Figure and Layout Audit}

Paper figures are generated as SVGs and converted to Wiley-preferred PDF graphics. The script \texttt{scripts/generate-interaction-figures.py} uses simple label-placement logic with visible callout lines so point labels remain distinct and connected to the corresponding point. The script \texttt{scripts/audit-paper-layout.py} checks the generated PDFs for sparse float pages, excessive vertical whitespace, and figure or table pages with too little surrounding readable text. Its report is written to \texttt{reports/paper-layout-audit.md}. A separate pre-submission human checklist is written to \texttt{reports/manual-visual-audit.md}; it tracks whether each figure label and each table float has been visually reviewed after regenerated artifacts.
