...
Table~\ref{tab:validation-gaps} reports the source-panel inventory that constrains the article's empirical scope. The main text reports categorical snapshot support so readers do not mistake narrow bridge moments for calibrated empirical estimates; numeric source values remain in the source-panel inventory report. The schema-scaffold column shows where mechanisms are exercised but not yet supported by representative public-data coverage. The validation reports also mark \texttt{source\_gap} rows when a panel is too narrow to test a benchmark directly, such as EPA-only procurement concentration or award-level rows used against post-award modification benchmarks. A fixture-only, thin, warning, missing, or \texttt{source\_gap} status is a validation gap; it should not be interpreted as evidence that the mechanism is unimportant in the real world.
