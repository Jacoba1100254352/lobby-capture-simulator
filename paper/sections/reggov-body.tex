...
Table~\ref{tab:substitution-warnings} ranks rows flagged by the warning rule defined above. Figure~\ref{fig:substitution-warning-map} maps observed-capture change against total-distortion change relative to the comparison row, with the upper-left quadrant marking cases where observed capture falls while total distortion rises.
...
The portfolio screen compares candidate reform bundles under the stress-test design. The portfolios are organized around transparency-first disclosure, balanced compliance, electoral substitution shielding, rulemaking integrity, procurement hardening, countervailing representation, high-deterrence enforcement, civil-liberties-constrained enforcement, and full anti-substitution design. The criterion minimizes total influence distortion first, then hidden capture, substitution risk, administrative burden, network opacity, legitimate-advocacy chill, and speech-restriction risk, while rewarding cross-venue detection and participation protection. Because those weights are normative, the screen should be read as a design diagnostic rather than a policy ranking.

\input{tables/portfolio_snapshot.tex}

\section{Validation Plan}

Validation is organized as schema verification, source-moment checks, scenario diagnostics, and calibration-gap tracking. The article relies on these checks to verify implementation consistency and identify empirical gaps; it does not treat them as validation of reform effects. Detailed validation targets, report artifacts, and calibration queues are documented in the supporting information \citep{ldaData,fecData,federalRegisterApi,regulationsGovApi,nycCfbData,irsEoBmf,seattleVouchers,usaspending}.

\section{Discussion}

The model-mode comparison shows what the single-channel baseline misses: reform diagnostics change once organized interests can move through alternative messengers, venues, financing routes, and intermediaries. The most robust reform features in the stress tests are not isolated disclosure rules, but combinations that pair visibility with enforcement capacity, countervailing representation, procurement safeguards, and cross-venue detection. The most important failure modes are evasion freedom, opaque intermediary paths, defensive mobilization against reform, and administrative burden that can weaken legitimate participation. Some strong-reform rows still sit in a near-saturated observed-capture region, so the clearest result is the substitution pattern conditional on that stress-test design.

\input{figures/scenario_tradeoffs}

This framing fits a regulatory-governance audience because the unit of analysis is not a single vote or donation, but a design problem across institutional venues. Transparency systems, countervailing participation, procedural-integrity rules, enforcement budgets, procurement firewalls, and revolving-door controls are modeled as interacting features of a governance architecture. The policy-relevant output is a map of where influence moves when a rule binds.

\section{Limitations and Future Validation}

The main limitation is evidentiary rather than computational. The 2024 EPA/ENV slice is useful for schema validation and distributional diagnostics, yet it remains incomplete for direct hidden-donor/source-routing observations, representative national public-financing program data, full IRS 527/Form 990 intermediary routing, multi-agency SAM/FPDS procurement action identifiers, and richer revolving-door mechanisms beyond LDA covered-position rows. A stronger validation cycle would broaden the FEC panel beyond national party committees and Schedule E, add direct public-financing and dark-money/electioneering sources, replace capacity proxies with documented expenditure/routing exports where possible, and keep procurement concentration separate from simulated procurement bias.
