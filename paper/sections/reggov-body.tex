...
Normalized source rows constrain only distributional inputs and warning diagnostics. LDA rows constrain issue funding scale, covered-official visibility, agency contact visibility, disclosure lag, and an LDA-derived covered-position revolving-door panel. FEC rows include national party committee receipts plus OpenFEC Schedule E independent expenditures and, when enabled, electioneering and communication-cost rows; they constrain donor concentration, outside-spending exposure, electoral-communication coverage, and traceability when the relevant panels are present. Public-financing rows include NYC CFB public-funds payments and Seattle Democracy Voucher aggregates, and dark-money or opaque-capacity bridge rows are handled as separate panels rather than silently mixed into ordinary FEC receipts. Source moments now separate IRS EO BMF opaque-capacity rows from non-proxy direct dark-money routing rows; the committed snapshot has zero direct routing rows. Federal Register and Regulations.gov rows constrain docket size, comment authenticity, template saturation, comment flooding, and technical-claim credibility. USAspending rows constrain procurement identifiers, competition fields, recipient concentration, and agency concentration. Procurement source moments separate the concentration denominator, award-level competition rows, a top-award bridge, and a stratified 12-agency quarterly transaction/action panel for modification diagnostics. The procurement denominator audit now reports both row-count balance and amount concentration, because a balanced action sample can still be nonrepresentative for volume-weighted SAM/FPDS calibration. A separate procurement-modification composition audit decomposes the high modified-action share by source route, agency, award type, and recipient concentration while keeping the representative SAM/FPDS rate claim blocked. The codebase also includes an opt-in SAM.gov Contract Awards importer for the same procurement action schema, but the committed snapshot does not promote that path until a representative action-history run is archived. When the action panel is present, it is preferred over the top-award bridge for concentration moments; neither panel is treated as a representative SAM/FPDS denominator. The EPA award slice remains the environmental calibration panel, while separate multi-agency bridge and action panels supply broader procurement diagnostics. Intermediary rows include local campaign-intermediary records, nonprofit and association capacity proxies, and bounded IRS Form 8872 rows for 527 political organizations. The intermediary bridge audit separates those rows from Form 990 nonprofit-routing evidence and reports zero Form 990 routing rows in the committed snapshot. Revolving-door rows remain proxy-backed through LDA covered-position reports; the revolving-door bridge audit separates those rows from documented post-employment personnel-movement evidence and reports zero documented movement rows in the committed snapshot.

\input{tables/validation_gap_snapshot.tex}

The main text reports categorical snapshot support so readers do not mistake narrow bridge moments for calibrated empirical estimates; numeric source values remain in the source-panel inventory report. The schema-scaffold column shows where mechanisms are exercised but not yet supported by representative public-data coverage. The validation reports also mark \texttt{source\_gap} rows when a panel is too narrow to test a benchmark directly, such as a bounded procurement concentration panel used against a representative multi-agency benchmark or a stratified USAspending transaction panel used against a representative SAM/FPDS modification-incidence benchmark. A fixture-only, thin, warning, missing, or \texttt{source\_gap} status is a validation gap; it should not be interpreted as evidence that the mechanism is unimportant in the real world.

The generated claim-boundary audit maps each source panel to the strongest manuscript claim it can support. Panels classified as thin, warning-level, fixture-only, or missing are limited to mechanism diagnostics, schema tests, or future-validation targets unless a later snapshot supplies stronger direct evidence.

The committed snapshot is a closed-window reproducibility scaffold for the 2024 EPA/ENV slice, not a representative empirical panel. Report builds default to the committed snapshot directory, \path{data/snapshots/2024-env/normalized/}. Exploratory runs can redirect calibration with \path{LOBBY_CAPTURE_CALIBRATION_DIR}. The validation snapshot adds Schedule E outside spending, OpenFEC electioneering and communication-cost rows, NYC CFB public-financing and intermediary rows, Seattle Democracy Voucher rows, IRS EO BMF nonprofit and association capacity rows, bounded IRS Form 8872 rows for 527 political organizations, IRS EO BMF opaque-capacity proxy rows, USAspending competition fields, UEI and PIID coverage, a multi-agency USAspending top-award bridge, a stratified 12-agency quarterly USAspending transaction/action panel, optional separate SAM.gov Contract Awards rows when configured, and LDA rows that identify covered positions in lobbying reports. The snapshot still lacks direct hidden-donor/source-routing observations, representative national public-financing panels, representative SAM/FPDS action-level procurement modification data, representative Form 990 nonprofit routing, complete IRS 527 coverage beyond the bounded alphabetic slice, and representative post-employment personnel-movement exports.
...
The main limitation is evidentiary rather than computational. The 2024 EPA/ENV slice supports schema validation and distributional diagnostics, and the multi-agency USAspending bridge plus stratified transaction/action panel improve procurement coverage. The source base remains incomplete for direct hidden-donor/source routing, representative national public financing beyond the local NYC and Seattle program rows, representative Form 990 nonprofit routing, complete IRS 527 coverage beyond the bounded alphabetic slice, representative SAM/FPDS procurement action histories, and revolving-door mechanisms beyond LDA covered-position rows. A stronger validation cycle would broaden the FEC panel beyond national party committees, Schedule E, and the bounded electoral-communication bridge; add federal, state, and additional local public-financing sources plus direct dark-money sources; replace capacity proxies with documented routing exports; and keep procurement concentration separate from simulated procurement bias.
