...
USAspending rows constrain procurement identifiers, competition fields, recipient concentration, and agency concentration. Procurement source moments separate award-level competition rows, a top-award bridge, national-volume and stratified USAspending action panels, the archived USAspending bulk transaction summary, and optional SAM.gov Contract Awards rows. The archived bulk summary now supplies the preferred public transaction-history denominator for configured agencies. Modification diagnostics are reported as transaction-row incidence, distinct-award incidence, and amount-weighted incidence, because each denominator answers a different source-bridge question. The procurement denominator and composition audits show that the bulk summary improves public coverage but does not by itself settle the benchmark mapping: calibrated procurement-modification claims still require remapping by action-row, distinct-award, amount-weighted, agency-mix, fiscal-year, award-type, and SAM/FPDS coding definitions. The codebase includes an opt-in SAM.gov Contract Awards importer for the same procurement action schema, with synchronous offset page-index strata and asynchronous JSON/CSV extract mode for keyed refreshes; that path is now most useful for coding crosswalks, exclusions, offer counts, protests, and firewall overlays. The EPA award slice remains the environmental calibration panel, while the multi-agency bridge, action panels, and bulk summary supply broader procurement diagnostics without being treated as calibrated SAM/FPDS modification estimates.
...
The main text reports categorical snapshot support so readers do not mistake narrow bridge moments for calibrated empirical estimates; numeric source values remain in the source-panel inventory report. The schema-scaffold column shows where mechanisms are exercised but not yet supported by representative public-data coverage. The validation reports also mark \texttt{source\_gap} rows when the available source denominator does not match the benchmark definition, such as the USAspending bulk transaction summary used against a SAM/FPDS-style modification-incidence benchmark. A fixture-only, thin, warning, missing, or \texttt{source\_gap} status is a validation gap; it should not be interpreted as evidence that the mechanism is unimportant in the real world.
