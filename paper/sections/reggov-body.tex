...
The third limitation is calibration coverage. Remaining P3 partial overlaps concentrate in hidden-influence stress-family splitting, public-financing candidate uptake, and regulator-backlog scope. These rows do not create hard source gaps for the mechanism article, but they identify where a later calibrated policy-simulation version would need sharper scenario families, stronger public-financing panels, and enforcement workload data. Additional empirical work would split source moments from simulated outcomes, tune comment compression against docket-level data, calibrate detection and sanction incidence against enforcement records, and anchor intermediary and revolving-door substitution with stronger panels. These steps would move the model from mechanism checking toward calibrated policy simulation.
