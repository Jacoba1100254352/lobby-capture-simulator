...
public final class CsvReportWriter
{
	...
	public String write(List<ScenarioReport> reports, ReportProvenance provenance) {
		...
		builder.append("generatedAt,seed,runs,contestsPerRun,scenarioKey,scenarioName,totalContests,capturedContests,antiCaptureReforms,enactedAntiCaptureReforms,directionalScore,captureControl,representation,reformFeasibility,captureRate,captureRateLower95,captureRateUpper95,antiCaptureSuccess,antiCaptureSuccessLower95,antiCaptureSuccessUpper95,observedCaptureRate,hiddenCaptureIndex,totalInfluenceDistortion,substitutionRisk,designLoss,distortionObservedComponent,distortionHiddenSubstitutionComponent,distortionInformationProcurementComponent,distortionNetworkVenueComponent,distortionProcessBurdenComponent,distortionRawComponentSum,distortionEqualWeightDiagnostic,distortionHiddenDiscountedDiagnostic,designLossBurdenHeavy,captureRateSeedStdDev,hiddenInfluenceSeedStdDev,totalInfluenceDistortionSeedStdDev,avgCaptureIndex,avgPublicInterest,publicPreferenceDistortion,privateGainRatio,lobbySpendPerContest,clientFundingPerContest,donorInfluenceGini,averageDisclosureLag,lobbyingDisclosureLag,campaignDisclosureLag,defensiveReformSpendShare,captureReturnOnSpend,publicBenefitPerInfluenceDollar,visibleLobbyingSpendShare,directAccessShare,agendaAccessShare,informationDistortionShare,publicCampaignShare,litigationThreatShare,campaignFinanceShare,darkMoneyShare,revolvingDoorShare,intermediaryShare,defensiveChannelShare,detectionRate,reportingErrorDetectionRate,campaignSanctionIncidence,sanctionRate,enforcementCapacityIndex,policyDistortion,regulatoryDrift,enforcementForbearance,procurementBias,darkMoneyTraceability,darkMoneyDirectVisibility,largeDonorDependence,voucherParticipation,voucherResidentParticipation,publicFinancingShare,publicFinancingCandidateUptake,revolvingDoorInfluence,commentRecordDistortion,commentAuthenticity,templateCommentSaturation,commentUniqueInformationShare,commentReviewBurden,commentProceduralAckRate,commentSubstantiveUptake,commentCompressionRate,commentFloodingIndex,technicalClaimCredibility,technicalRulemakingDistortion,channelSwitchRate,evasionShiftRate,evasionPenaltyRate,substitutionPressure,influencePreservationRate,hiddenInfluenceShare,netTransparencyGain,messengerSubstitutionRate,venueSubstitutionRate,clientFundingAdaptation,regulatorAttentionIndex,regulatorQueueBacklog,watchdogFocusIndex,watchdogBudgetConcentration,adaptationSpeed,reformDecayPressure,legitimateAdvocacyChill,constitutionalChallengeDelay,administrativeCost,networkOpacityIndex,donorNetworkConcentration,intermediaryCentrality,officialAccessCentrality,procurementNetworkExposure,revolvingDoorBridgeIndex,commentNetworkLoad,venueShiftNetworkLoad,networkLegibilityIndex,crossVenueDetectionIndex,participationProtectionIndex,speechRestrictionRisk,switchScoreEqualWeight,switchScoreAnonymityHeavy,switchScoreEnforcementCostHeavy,switchDisclosureCost\n");
		for (ScenarioReport report : reports) {
			...
			builder.append(provenance.generatedAt()).append(',')
			       ...
			       .append(format(antiCaptureInterval[0])).append(',')
				       ...
				       .append(format(report.observedCaptureRate())).append(',')
				       .append(format(report.hiddenCaptureIndex())).append(',')
				       .append(format(report.totalInfluenceDistortion())).append(',')
				       .append(format(report.substitutionRisk())).append(',')
				       .append(format(report.designLoss())).append(',')
				       .append(format(report.distortionObservedComponent())).append(',')
				       .append(format(report.distortionHiddenSubstitutionComponent())).append(',')
				       .append(format(report.distortionInformationProcurementComponent())).append(',')
			       .append(format(report.distortionNetworkVenueComponent())).append(',')
			       .append(format(report.distortionProcessBurdenComponent())).append(',')
			       .append(format(report.distortionRawComponentSum())).append(',')
			       .append(format(report.distortionEqualWeightDiagnostic())).append(',')
			       .append(format(report.distortionHiddenDiscountedDiagnostic())).append(',')
			       .append(format(report.designLossBurdenHeavy())).append(',')
			       .append(format(report.captureRateSeedStdDev())).append(',')
			       .append(format(report.hiddenInfluenceSeedStdDev())).append(',')
			       .append(format(report.totalInfluenceDistortionSeedStdDev())).append(',')
			       ...
			       .append(format(report.lobbyingDisclosureLag())).append(',')
			       .append(format(report.campaignDisclosureLag())).append(',')
			       ...
			       .append(format(report.visibleLobbyingSpendShare())).append(',')
			       ...
			       .append(format(report.intermediarySpendShare())).append(',')
			       .append(format(report.defensiveChannelSpendShare())).append(',')
			       .append(format(report.detectionRate())).append(',')
			       .append(format(report.reportingErrorDetectionRate())).append(',')
			       .append(format(report.campaignSanctionIncidence())).append(',')
			       .append(format(report.sanctionRate())).append(',')
			       .append(format(report.enforcementCapacityIndex())).append(',')
			       ...
			       .append(format(report.commentFloodingIndex())).append(',')
			       .append(format(report.technicalClaimCredibility())).append(',')
			       .append(format(report.technicalRulemakingDistortion())).append(',')
			       ...
			       .append(format(report.networkOpacityIndex())).append(',')
			       .append(format(report.donorNetworkConcentration())).append(',')
			       .append(format(report.intermediaryCentrality())).append(',')
			       .append(format(report.officialAccessCentrality())).append(',')
			       .append(format(report.procurementNetworkExposure())).append(',')
			       .append(format(report.revolvingDoorBridgeIndex())).append(',')
			       .append(format(report.commentNetworkLoad())).append(',')
			       .append(format(report.venueShiftNetworkLoad())).append(',')
			       .append(format(report.networkLegibilityIndex())).append(',')
			       .append(format(report.crossVenueDetectionIndex())).append(',')
			       .append(format(report.participationProtectionIndex())).append(',')
			       .append(format(report.speechRestrictionRisk())).append(',')
			       .append(format(report.switchScoreEqualWeight())).append(',')
			       .append(format(report.switchScoreAnonymityHeavy())).append(',')
			       .append(format(report.switchScoreEnforcementCostHeavy())).append(',')
			       .append(format(report.switchDisclosureCost())).append('\n');
		}
