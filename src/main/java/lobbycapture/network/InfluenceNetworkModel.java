...
public final class InfluenceNetworkModel {
    ...
    private static void add(
        ...
        paths.add(new InfluenceNetworkPath(
                ...
                channel,
                weight,
                opacity(channel, group, reform, world),
                donorConcentration(channel, group, donorInfluenceGini),
                intermediaryDependence(channel, group),
    ...
    private static double opacity(InfluenceChannel channel, LobbyOrganization group, ReformRegime reform, WorldState world) {
    ...
    private static double intermediaryDependence(InfluenceChannel channel, LobbyOrganization group) {
        double channelPressure = switch (channel) {
        ...
        return Values.clamp(channelPressure + (0.16 * group.researchBudget() / Math.max(1.0, group.totalBudget())), 0.0, 1.0);
    }
    ...
    private static double procurementLink(InfluenceChannel channel, LobbyOrganization group, PolicyContest contest, ReformRegime reform) {
        ...
        return Values.clamp(domainPressure + channelPressure + (0.18 * group.preferenceFor("procurement")) - (0.24 * reform.blindReviewStrength()), 0.0, 1.0);
    }
    ...
    private static double commentMobilization(InfluenceChannel channel, LobbyOrganization group, PolicyContest contest, ReformRegime reform) {
