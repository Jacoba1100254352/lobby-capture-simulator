...
def bulk_procurement_moments(scope: str, summary: dict[str, object]) -> list[dict[str, str]]:
    ...
    return [
        ...
        moment(scope, "usaspending-bulk", "procurementProtestShare", number(summary.get("protestShare")), "diagnostic", "legacy bulk protest flag defaults false without protest-source evidence; not an observed no-protest rate"),
        moment(scope, "usaspending-bulk", "procurementExclusionShare", number(summary.get("exclusionShare")), "observed_proxy", "competition text containing exclusion in the archived bulk collector; not historical SAM vendor exclusion/debarment status"),
        moment(scope, "usaspending-bulk", "procurementFirewallCoverageShare", number(summary.get("firewallCoverageShare")), "diagnostic", "legacy bulk firewall flag defaults false without an observed overlay; not evidence of absent safeguards"),
