...
def write_model_architecture(figure_dir: Path) -> None:
    ...
    def add_arrow(x1: float, y1: float, x2: float, y2: float) -> None:
        ...
        if abs(x2 - x1) >= abs(y2 - y1):
            ...
            body.append(line(x2 - direction * 20, y2 - 14, x2, y2, "axis"))
            body.append(line(x2 - direction * 20, y2 + 14, x2, y2, "axis"))
        else:
            direction = 1 if y2 >= y1 else -1
            body.append(line(x2 - 14, y2 - direction * 20, x2, y2, "axis"))
            body.append(line(x2 + 14, y2 - direction * 20, x2, y2, "axis"))

    boxes = [
        ("Funders and clients", 70, 235, "Private gain,\nexposure,\nsecrecy need"),
        ("Lobby organizations", 500, 235, "Budgets,\nskills,\nstrategy memory"),
        ("Influence channels", 930, 235, "Access, finance,\ncomments,\nintermediaries"),
        ("Institutional arenas", 1360, 235, "Elections,\nrulemaking,\nprocurement"),
        ("Outcomes and metrics", 1360, 535, "Observed capture,\nhidden capture,\ntotal distortion"),
    ]
    for title, x, y, detail in boxes:
        body.append(rect(x, y, 370, 170, "#f7f7f7", "arch-box"))
        body.append(text(x + 185, y + 47, title, anchor="middle", css_class="arch-title"))
        for line_index, line_text in enumerate(detail.split("\n")):
            body.append(text(x + 185, y + 88 + line_index * 32, line_text, anchor="middle", css_class="small"))

    add_arrow(440, 320, 500, 320)
    add_arrow(870, 320, 930, 320)
    add_arrow(1300, 320, 1360, 320)
    add_arrow(1545, 405, 1545, 535)

    body.append(rect(360, 805, 1080, 135, "#ffffff", "arch-box"))
    body.append(text(900, 856, "Reforms alter channel costs, visibility, enforcement, and participation capacity", anchor="middle", css_class="arch-title"))
    body.append(text(900, 902, "Substitution moves influence across messengers, venues, financing routes, and technical records", anchor="middle", css_class="small"))
    body.append(line(685, 805, 685, 405, "threshold"))
    body.append(line(1545, 805, 1545, 705, "threshold"))
...
def write_substitution_warning_map(rows: list[dict[str, str]], report: Path, figure_dir: Path) -> None:
    ...
    for index, row in enumerate(selected, start=1):
        ...
        if index > 5:
            label = f"{index}. {label}"
        label_targets.append(LabelTarget(label, x, y))
...
def write_wrapper(
    ...
    if floating:
        body = [
            ...
            "\\begin{figure}[htbp]",
            "\\centering",
            f"\\includegraphics[width={width}]{{figures/{pdf_name}}}",
            f"\\caption{{{caption}}}",
            f"\\label{{{label}}}",
            "\\end{figure}",
            "",
