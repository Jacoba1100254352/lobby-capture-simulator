...
def write_markdown(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        (
            ...
            "SAM.gov emailed async-extract links with `api_key=REPLACE_WITH_API_KEY` "
            "should be recorded with `make sam-contract-awards-record-export-link < sam-email.txt` "
            "or `python3 scripts/record-sam-export-link.py --url ...`; the helper writes "
            "expiration metadata, records whether timing came from an email `Date` header, "
            "an explicit timestamp, or an intentional `--assume-fresh` body-only fallback, clears stale "
            "`SAM_CONTRACT_AWARDS_LIVE_CSV` overrides, and keeps the private key in "
            "`SAM_API_KEY`. Body-only pasted emails with no `Date:` header fail closed unless "
            "recorded through `make sam-contract-awards-record-fresh-link` or `--assume-fresh`; "
            "if the helper reports `timeSource=recorded_at_fallback`, "
            "treat the link as usable only when the email was just generated; otherwise "
            "request a fresh export email. The local `SAM_API_KEY` is "
            "substituted at runtime and redacted from diagnostics. "
