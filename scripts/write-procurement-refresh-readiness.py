...
def write_markdown(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        (
            ...
            "an explicit timestamp, or a record-time fallback, clears stale "
            "`SAM_CONTRACT_AWARDS_LIVE_CSV` overrides, and keeps the private key in "
            "`SAM_API_KEY`. If the helper reports `timeSource=recorded_at_fallback`, "
            "treat the link as usable only if the email was just generated; otherwise "
