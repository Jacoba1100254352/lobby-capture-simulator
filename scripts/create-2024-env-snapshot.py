...
SOURCES = {
    ...
    "oira-meetings": {
        "input": RAW / "oira-meetings.csv",
        "description": "Reginfo.gov EO 12866 public meeting disclosures for meeting-date, agency, RIN, rule-title, requestor, and requestor-client visibility diagnostics.",
        "request": "REGINFO_EO_MEETINGS_MAX_ROWS=100 REGINFO_EO_MEETINGS_DETAIL_LIMIT=25 python3 scripts/fetch-source-data.py oira-meetings --output data/raw/oira-meetings.csv",
    },
    "usaspending": {
...
def source_status(source: str, live_status: list[dict[str, str]]) -> tuple[str, str]:
    ...
    names = {
        ...
        "oira-meetings": [row for row in live_status if row["source"] == "oira-meetings"],
