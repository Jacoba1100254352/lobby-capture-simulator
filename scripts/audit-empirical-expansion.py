...
def audit():
    ...
    add("expanded-lda-history", "api_attribution_identity_unresolved",
        ...
        "Counts reflect source/API attribution, not validated actor histories. Review of the 21 frozen NVG records finds six America Votes filings ineligible for AAJ attribution, including one registration. Five mixed-client groups are not mergeable; two cover years disagree with API years. Validate complete histories, amendments and exact identities; pre-period counts are native reporting periods, not independent quarters or proof of completeness.")
    ...
    add("alternate-channel-coverage", "partial_outcome_coverage" if unresolved else "complete_source_periods_not_design",
        ...
        "Keep all declared acquisition-frame periods visible. Benefits Council version flags are reconciled through a source-bound reports/filings join; missing financial fields, scan/API amount differences and a header/itemized-date conflict remain. The raw-date gap is not evidence of absent activity. AAJ 2008H2 uses a source-bound supplemental-loan-paperwork adjudication; independent review remains pending. Missing periods are not zero spending.")
    ...
    publisher_infrastructure = json.loads((DATA / "comment-publisher-infrastructure-review.json").read_text(encoding="utf-8"))
