...
CATEGORY_BY_METRIC = {
    ...
    "detectionRate": ("benchmark-review", "keep broad modeled capture detection separate from source-specific reporting-error benchmarks"),
    "reportingErrorDetectionRate": ("model-tuning", "tune the narrow reporting-error detection surface if it leaves the source reporting-error range"),
    "campaignSanctionIncidence": ("model-tuning", "tune the campaign-filer sanction surface if it leaves the source campaign-sanction range"),
    "sanctionRate": ("benchmark-review", "keep broad per-contest capture sanctions separate from campaign-filer sanction incidence"),
    ...
    "procurementAgencyTop1Share": ("direct-source-moment", "use the national USAspending action panel for concentration diagnostics and representative SAM/FPDS extracts before treating agency concentration as calibrated"),
    "procurementRecipientTop3Share": ("direct-source-moment", "use the national USAspending action panel for concentration diagnostics, then broaden by award type and fiscal year before treating recipient concentration as calibrated"),
    ...
    "procurementExPostModificationShare": ("direct-source-moment", "broaden the bounded USAspending action panel with representative SAM/FPDS action histories or archived USAspending bulk transaction downloads that support transaction-row, distinct-award, and amount-weighted denominators before treating modification incidence as calibrated"),
